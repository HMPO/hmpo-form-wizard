...
var debug = require('debug')('hmpo:progress-check');

module.exports = function checkProgress(route, controller, steps, start) {
  start = start || '/';

  var previousSteps = _.reduce(steps, function getPreviousSteps(list, step, path) {
    if (step.next === route) {
      list.push(path);
    }
    return list;
  ...
  var invalidatingFields = _.pick(controller.options.fields, function getInvalidatingFields(f) {
    return f && f.invalidates && f.invalidates.length;
  });

  var getLaterSteps = function getLaterSteps(path) {
    var s = [];
    while (path) {
      s.push(path);
  ...
  controller.on('complete', function onComplete(req, res, path) {
  ...
  return function checkProgressMiddleware(req, res, next) {
    _.each(invalidatingFields, function eachInvalidatingField(field, key) {
      req.sessionModel.on('change:' + key, function onChange() {
        ...
        var allSteps = _.difference(req.sessionModel.get('steps'), laterSteps);
        req.sessionModel.set('steps', allSteps);
      });
    });
