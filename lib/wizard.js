...
let wizard = function (steps, fields, wizardOptions) {
    ...
    let processedSteps = _.mapObject(steps, (options, route) => {
        ...
        if (Array.isArray(options.fields)) {
            options.fields = _.object(  // Convert fields array to object
                options.fields, // Keys (field names)
                _.map(options.fields, fieldKey => fields[fieldKey] || {}) // Values (field definitions)
            );
        }
