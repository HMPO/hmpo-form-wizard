...
module.exports = Controller => class extends Controller {
    ...
    checkSkipStep(req) {
        const { skipStep } = req.form.options;
        debug('%s Check skip step', req.form.options.fullPath, skipStep);

        const skip = (typeof skipStep === 'boolean' && skipStep === true) ||
            (typeof skipStep === 'function' && skipStep(req));
        if (skip) {
            debug('%s Skipping step', req.form.options.fullPath);
        }
    ...
    setStepComplete(req, res, path) {
        ...
        let newItem = {
            ...
            skipStep: this.checkSkipStep(req),
            editing: req.isEditing && !req.notRevalidated ? true : undefined,
            continueOnEdit: req.isEditing && !req.notRevalidated ? continueOnEdit :  undefined
        };
